    ...
    with tqdm(total=100, position=1, bar_format='{desc}', desc='Stats') as desc:
        for epoch in tqdm(range(epoch_num)):
            ...
            if (epoch+1) % show_log_freq == 0:
                ...
                writer.add_scalar('Balance', (env.balance+env.position_value_abs), epoch+1)
                desc.set_description(' | '.join(map(str, [epoch+1, f'step {total_step}', f'reward {log_reward}', f'loss {log_loss}', f'value {env.balance + env.position_value_abs}', f'time {elapsed_time}'])))
                start = time.time()
